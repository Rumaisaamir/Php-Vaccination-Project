...
<!-- <div class="alert alert-success" role="alert" style="max-width: 820px; margin: 0 auto; border: 0; border-radius: 0; height: 50px; display: flex; align-items: center;">
  This is a success!
        ...
        if(isset($_SESSION['approval_message'])) {
            echo '<div class="alert alert-success" role="alert" style="max-width: 820px; margin: 0 auto; border: 0; border-radius: 0; height: 50px; display: flex; align-items: center; justify-content: center">' . $_SESSION['approval_message'] . '</div>';
            unset($_SESSION['approval_message']); // Clear the session variable
        }
        ?>
...
$approve_appointment_search_query = isset($_GET['approve_appointment_search_query']) ? $_GET['approve_appointment_search_query'] : '';
$approve_appointment_search_query = mysqli_real_escape_string($con, $approve_appointment_search_query);

$get_approve_appointment = "SELECT * FROM users WHERE status = 'Approved' AND appointment = 'Pending' AND report = 'Inprocess' AND Preferred_Center = '$Hospital_Name'";

// Modify the SQL query to search for appointment IDs starting with the given input
if (!empty($approve_appointment_search_query)) {
    $get_approve_appointment .= " AND appointment_id LIKE '$approve_appointment_search_query%'";
}
$approve_appointment_result = $con->query($get_approve_appointment);

?>

<div style="margin-bottom:-25px;" class="section-title">
    <h2 id="h2_ki_id_doctor_2" style="color:#55a1e4;">Awaiting Vaccinations</h2>
</div>

<div id="generate_div">
    <form method="get">
        <div style="display: flex;">
            <input type="search" id="generate_div_searchbar" name="approve_appointment_search_query" placeholder="Enter Appointment No." value="<?php echo isset($approve_appointment_search_query) ? $approve_appointment_search_query : ''; ?>" style="width: 750px; padding: 5px; border: 1px solid #1977cc; border-radius: 3px;" required>
...
<div id="appointment_table_div_doctor_2" class="table-container" style="width: 100%; overflow-x: auto;">
    <table class="table" style="width: 100%; white-space: nowrap;">
        ...
        <tbody>
<?php
    if ($approve_appointment_result->num_rows > 0) {
    while ($row = $approve_appointment_result->fetch_assoc()) {
    $vaccine_status = $row['appointment'];
?>
    <tr>
        ...
        <td style="border: 1px solid lightgrey; padding: 8px; text-align: center;">
            <form action="vaccinated.php" method="POST">
                    <input type="hidden" name="id" value="<?php echo $row['UserID']; ?>"/>
                    <input title="Vaccinated" style="padding:0px 15px 0px 15px; border-radius: 0px;" id="approve_btns" type="submit" class="btn" name="vaccinated" value="Vaccinated"> 
            </form>
        </td>
    </tr>
<?php
...
<?php
if (!empty($approve_appointment_search_query) && $approve_appointment_result->num_rows == 0) {
    echo '<div id="appointment_table_div_doctor_2" class="table-container" style="width: 100%; overflow-x: auto;">
            <table class="table" style="width: 100%; white-space: nowrap;">
                <thead class="table-danger border">
                    <tr>
                        <th style="color: red;" colspan="12">No appointment found for ID: ' . htmlentities($approve_appointment_search_query) . '</th>
                    </tr>
                </thead>
            </table>
          </div>';
