...
<?php
        if(isset($_SESSION['vaccinated_message'])) {
            echo '<div class="alert alert-success" role="alert" style="max-width: 820px; margin: 0 auto; border: 0; border-radius: 0; height: 50px; display: flex; align-items: center; justify-content: center">' . $_SESSION['vaccinated_message'] . '</div>';
            unset($_SESSION['vaccinated_message']); // Clear the session variable
        }
?>
<br>
...
$generate_search_query = isset($_GET['generate_search_query']) ? $_GET['generate_search_query'] : '';
$generate_search_query = mysqli_real_escape_string($con, $generate_search_query);

$generate_appointment = "SELECT * FROM users WHERE status = 'Approved' AND  appointment = 'Vaccinated' AND report = 'Inprocess' AND Preferred_Center = '$Hospital_Name'";

if (!empty($generate_search_query)) {
    $generate_appointment .= " AND appointment_id LIKE '$generate_search_query%'";
}
$appointment_report_result = $con->query($generate_appointment);
?>

<div style="margin-bottom:-25px;" class="section-title">
<h2 id="h2_ki_id_doctor_2" style="color:#55a1e4;">Generate Vaccinations Report</h2>
</div>

<div id="generate_div">
    <form method="get">
        <div style="display: flex;">
            <input type="search" id="generate_div_searchbar" name="generate_search_query" placeholder="Enter Appointment No." value="<?php echo isset($generate_search_query) ? $generate_search_query : ''; ?>" style="width: 750px; padding: 5px; border: 1px solid #1977cc; border-radius: 3px;" required>
...
<div id="appointment_table_div_doctor_2" class="table-container" style="width: 100%; overflow-x: auto;">
    <table class="table" style="width: 100%; white-space: nowrap;">
        ...
        <th scope="col" style="background-color: #83baeb; color:white; border-right: 1px solid #83baeb; padding: 8px;">Action</th>
        </tr>
        </thead>
        <tbody>
        <?php
            if ($appointment_report_result->num_rows > 0) {
                while ($row = $appointment_report_result->fetch_assoc()) {
            ?>
        <tr>
        ...
        <a style="padding:0px 5px 0px 5px; border-radius: 0px;" id="edit_btns" class="btn edit-button" title="Generate Report" href="report_generate_form.php?appointment_id=<?php echo urlencode($row['appointment_id']); ?>">Generate Report</a>
        </td>
        </tr>
        <?php
        }
...
<?php
if (!empty($generate_search_query) && $appointment_report_result->num_rows == 0) {
    echo '<div id="appointment_table_div_doctor_2" class="table-container" style="width: 100%; overflow-x: auto;">
            <table class="table" style="width: 100%; white-space: nowrap;">
                <thead class="table-danger border">
                    <tr>
                        <th style="color: red;" colspan="12">No appointment found for ID: ' . htmlentities($generate_search_query) . '</th>
                    </tr>
                </thead>
            </table>
          </div>';
