...
$all_appointments_search_query = isset($_GET['all_appointments_search_query']) ? $_GET['all_appointments_search_query'] : '';
$all_appointments_search_query = mysqli_real_escape_string($con, $all_appointments_search_query);

$get_appointment = "SELECT * FROM users WHERE Preferred_Center = '$Hospital_Name'";

if (!empty($all_appointments_search_query)) {
    $get_appointment .= " AND appointment_id LIKE '$all_appointments_search_query%'";
}
$appointment_result = $con->query($get_appointment);
?>

<div style="margin-bottom:-25px;" class="section-title">
    <h2 id="h2_ki_id_doctor_2" style="color:#55a1e4;">Total Appointments</h2>
</div>

<div id="generate_div">
    <form method="get">
        <div style="display: flex;">
            <input type="search" id="generate_div_searchbar" name="all_appointments_search_query" placeholder="Enter Appointment No." value="<?php echo isset($all_appointments_search_query) ? $all_appointments_search_query : ''; ?>" style="width: 750px; padding: 5px; border: 1px solid #1977cc; border-radius: 3px;" required>
...
<div id="appointment_table_div_doctor_2" class="table-container" style="width: 100%; overflow-x: auto; background:">
...
<?php
if (!empty($all_appointments_search_query) && $appointment_result->num_rows == 0) {
    echo '<div id="appointment_table_div_doctor_2" class="table-container" style="width: 100%; overflow-x: auto;">
            <table class="table" style="width: 100%; white-space: nowrap;">
                <thead class="table-danger border">
                    <tr>
                        <th style="color: red;" colspan="12">No appointment found for ID: ' . htmlentities($all_appointments_search_query) . '</th>
                    </tr>
                </thead>
            </table>
          </div>';
