...
<?php
        if(isset($_SESSION['generate_message'])) {
            echo '<div class="alert alert-success" role="alert" style="max-width: 820px; margin: 0 auto; border: 0; border-radius: 0; height: 50px; display: flex; align-items: center; justify-content: center">' . $_SESSION['generate_message'] . '</div>';
            unset($_SESSION['generate_message']); // Clear the session variable
        }
?>
...
$send_appointment_search_query = isset($_GET['send_appointment_search_query']) ? $_GET['send_appointment_search_query'] : '';
$send_appointment_search_query = mysqli_real_escape_string($con, $send_appointment_search_query);

$send_vaccinated_appointment = "SELECT * FROM vaccination_reports WHERE send_report = 'Unsent' And preferred_center = '$Hospital_Name'";

// Modify the SQL query to search for appointment IDs starting with the given input
if (!empty($send_appointment_search_query)) {
    $send_vaccinated_appointment .= " AND appointment_id LIKE '$send_appointment_search_query%'";
}
$send_appointment_result = $con->query($send_vaccinated_appointment);

?>

<div style="margin-bottom:-25px;" class="section-title">
<h2 id="h2_ki_id_doctor_2" style="color:#55a1e4;">Send Vaccinations Report</h2>
</div>

<div id="generate_div">
    <form method="get">
        <div style="display: flex;">
            <input type="search" id="generate_div_searchbar" name="send_appointment_search_query" placeholder="Enter Appointment No." value="<?php echo isset($send_appointment_search_query) ? $send_appointment_search_query : ''; ?>" style="width: 750px; padding: 5px; border: 1px solid #1977cc; border-radius: 3px;" required>
...
<div id="appointment_table_div_doctor_2" class="table-container" style="width: 100%; overflow-x: auto;">
    <table class="table" style="width: 100%; white-space: nowrap;">
        ...
        <th scope="col" style="background-color: #83baeb; color:white;  padding: 8px;">Result</th>
        <th scope="col" style="background-color: #83baeb; color:white; border-right: 1px solid #83baeb; padding: 8px;">Actions</th>
        </tr>
        </thead>
        <tbody>
<?php
if ($send_appointment_result->num_rows > 0) { 
while ($row = $send_appointment_result->fetch_assoc()) {
?>
        <tr>
        ...
        <td style="border: 1px solid lightgrey; padding: 8px;"><?php echo $row['dosage']; ?></td>
        <td style="border: 1px solid lightgrey; padding: 8px;"><?php echo $row['result']; ?></td>
        <td class="status" style="border-right: 1px solid lightgrey; padding: 8px; text-align: center;">
        <!-- <a style="padding:0px 5px 0px 5px; border-radius: 0px;" id="edit_btns" class="btn edit-button" title="Generate Report" href="report_generate_form.php?id=<?php echo $row['UserID']; ?>&Name=<?php echo urlencode($row['Name']); ?>&Age=<?php echo urlencode($row['Age']); ?>&appointment_id=<?php echo urlencode($row['appointment_id']); ?>&Date_of_Vaccination=<?php echo urlencode($row['Date_of_Vaccination']); ?>&Gender=<?php echo urlencode($row['Gender']); ?>&Contact_Number=<?php echo urlencode($row['Contact_Number']); ?>&Vaccination_Type=<?php echo urlencode($row['Vaccination_Type']); ?>&Dose=<?php echo urlencode($row['Dose']); ?>&Preferred_Center=<?php echo urlencode($row['Preferred_Center']); ?>">Generate Report</a> -->
        <a style="padding:0px 5px 0px 5px; border-radius: 0px;" id="edit_btns" class="btn edit-button" title="View Report" href="Send_Report_pdf.php?appointment_id=<?php echo urlencode($row['appointment_id']); ?>&action=view"><i class="fa fa-eye" aria-hidden="true"></i></a>
        <!-- <a style="padding:0px 5px 0px 5px; border-radius: 0px;" id="edit_btns" class="btn edit-button" title="View Report" href=""><i class="fa fa-edit" aria-hidden="true"></i></a> -->
        <a style="padding:0px 5px 0px 5px; border-radius: 0px;" id="edit_btns" class="btn edit-button" title="Download Report" href="Send_Report_pdf.php?appointment_id=<?php echo urlencode($row['appointment_id']); ?>&action=download"><i class="fa fa-download" aria-hidden="true"></i></a>
        <a style="padding:0px 5px 0px 5px; border-radius: 0px;" id="edit_btns" class="btn edit-button" title="Send Report" href="Send_Report_pdf.php?appointment_id=<?php echo urlencode($row['appointment_id']); ?>&action=send_email"><i class="fa fa-paper-plane" aria-hidden="true"></i></a>
        </td>
        </tr>
        <?php
        }
...
<?php
if (!empty($send_appointment_search_query) && $send_appointment_result->num_rows == 0) {
    echo '<div id="appointment_table_div_doctor_2" class="table-container" style="width: 100%; overflow-x: auto;">
            <table class="table" style="width: 100%; white-space: nowrap;">
                <thead class="table-danger border">
                    <tr>
                        <th style="color: red;" colspan="12">No appointment found for ID: ' . htmlentities($send_appointment_search_query) . '</th>
                    </tr>
                </thead>
            </table>
          </div>';
