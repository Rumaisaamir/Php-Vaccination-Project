...
<?php
        if(isset($_SESSION['send_message'])) {
            echo '<div class="alert alert-success" role="alert" style="max-width: 820px; margin: 0 auto; border: 0; border-radius: 0; height: 50px; display: flex; align-items: center; justify-content: center">' . $_SESSION['send_message'] . '</div>';
            unset($_SESSION['send_message']); // Clear the session variable
        }
?>
<br>
...
$vaccinated_appointment_search_query = isset($_GET['vaccinated_appointment_search_query']) ? $_GET['vaccinated_appointment_search_query'] : '';
$vaccinated_appointment_search_query = mysqli_real_escape_string($con, $vaccinated_appointment_search_query);

$get_vaccinated_appointment = "SELECT * FROM vaccination_reports WHERE send_report = 'send' AND Preferred_Center = '$Hospital_Name'";

// Modify the SQL query to search for appointment IDs starting with the given input
if (!empty($vaccinated_appointment_search_query)) {
    $get_vaccinated_appointment .= " AND appointment_id LIKE '$vaccinated_appointment_search_query%'";
}
$vaccinated_appointment_result = $con->query($get_vaccinated_appointment);

?>

<div style="margin-bottom:-25px;" class="section-title">
  <h2 id="h2_ki_id_doctor_2" style="color:#55a1e4;">Completed Vaccinations</h2>
</div>

<div id="generate_div">
    <form method="get">
        <div style="display: flex;">
            <input type="search" id="generate_div_searchbar" name="vaccinated_appointment_search_query" placeholder="Enter Appointment No." value="<?php echo isset($vaccinated_appointment_search_query) ? $vaccinated_appointment_search_query : ''; ?>" style="width: 750px; padding: 5px; border: 1px solid #1977cc; border-radius: 3px;" required>
  ...
  <table class="table" style="width: 100%; white-space: nowrap;">
    <thead class="table-dark border">
      <tr>
        ...
        <th scope="col" style="background-color: #83baeb; color:white; border-right: 1px solid #83baeb; padding: 8px;">Report Send</th>
      </tr>
    </thead>
<tbody>

<?php
if ($vaccinated_appointment_result->num_rows > 0) { 
while ($row = $vaccinated_appointment_result->fetch_assoc()) {
?>
      <tr>
...
<?php
if (!empty($vaccinated_appointment_search_query) && $vaccinated_appointment_result->num_rows == 0) {
    echo '<div id="appointment_table_div_doctor_2" class="table-container" style="width: 100%; overflow-x: auto;">
            <table class="table" style="width: 100%; white-space: nowrap;">
                <thead class="table-danger border">
                    <tr>
                        <th style="color: red;" colspan="12">No appointment found for ID: ' . htmlentities($vaccinated_appointment_search_query) . '</th>
                    </tr>
                </thead>
            </table>
          </div>';
